...
const Index = () => {
  useSeoMeta({
    ...
    description: 'Convert USD to Bitcoin using the UTXOracle on-chain price oracle.',
  });
  ...
  const handleUsdChange = useCallback(
    (raw: string) => {
      // allow digits, dot, and leading minus is disallowed
          ...
          <div className="flex items-center gap-2 flex-wrap justify-center">
            ...
            {data && (
              <span className="text-xs text-zinc-400 dark:text-zinc-500">
                Block #{data.block_height.toLocaleString()} · {formatTimeAgo(data.updated_at)}
              </span>
            )}
          </div>
        )}
